...
export default function Artists({ artists }) {
  ...
  return (
    <div className={`${styles.artists} bg-opaqueGray`}>
      <div className="w-fit flex items-center gap-14 mx-auto">
        <div
          className={`w-24 h-0.5 ${styles.gradientBorderStraightLeft} hidden sm:block`}
        ></div>
        <div className="grid grid-cols-3 gap-2 mx-auto max-w-[292px] py-12">
          {artists.map((artist, idx) =>
            artist.attributes.profile_image.data ? (
              <Link
                ...
                className={`${styles.gradientBorder} w-[92px] h-[92px] rounded-full p-1.5`}
              >
                <Image
          ...
          {artistNumber < 6 && artistNumber % 3 !== 0 ? (
            <div className="relative">
              <div
                className={`${styles.gradientBorder} w-[92px] h-[92px] rounded-full p-1.5 flex items-center justify-center opacity-20`}
              >
        ...
        <div
          className={`w-24 h-0.5 ${styles.gradientBorderStraightRight} hidden sm:block`}
        ></div>
      </div>
    </div>
  );
